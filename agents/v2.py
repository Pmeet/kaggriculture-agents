...
DEFAULTS = {
    ...
    "pickup_batch": 8,
    # --- Market.
    "sell_floor_frac": 0.60,
    "max_price_impact": 0.18,
    ...
    if not endgame and info["empty"]:
        ...
        for i, pos in enumerate(by_distance):
            if i < n_struct:
                ...
                add(pos, ["PLANT", crop_choice], max(0.0, tile_worth), "PLANT",
                    crop=crop_choice)
...
def assign_units(units, jobs, depots, hours_left, seeds, params, shed, endgame,
    ...
    for ui, (pos, inv) in enumerate(units):
        ...
        if ji is None:
            actions.append(idle_action(pos, inv, depots, shed_total, endgame, supplies))
            continue
        job = jobs[ji]
        item = job.get("need")
        if item and inv.get(item, 0) <= 0:
            ...
            if tuple(pos) == tuple(depot):
                batch = min(params["pickup_batch"], max(1, pickup_needs.get(item, 1)),
                            pool.get(item, 0))
                if batch <= 0:
                    actions.append(idle_action(pos, inv, depots, shed_total,
                                               endgame, supplies))
                    continue
...
def idle_action(pos, inv, depots, shed_total, endgame, supplies=()):
    ...
    if tuple(pos) == tuple(target):
        if endgame or shed_total + carrying <= SHED_CAPACITY:
            return ["DROP"]
        return ["PASS"]
...
def plan_market(obs, farm, private, params, shed, day, hour, step, info,
    ...
    # ---- Livestock last, and only into a pen that is free *and* unclaimed by
