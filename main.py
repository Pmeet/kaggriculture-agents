...
DEFAULTS = {
    ...
    # 11 was optimal when the farm ran five animals. Delivering the livestock we
    # already bought (v23) put fifteen on the board, and the extra four jobs per
    # animal per day moved the optimum: 13 measures +$1,849/+$3,209 against v23
    # over 20 seeds a side, +$1,181/+$2,532 over 32, and +$3,377 in a third run.
    # 15 and 17 are both worse (-$3,339 and -$6,389) -- hands cost fib(n) a day,
    # so the 14th and 15th cost $610 and $987 each on their own.
    "max_hands": 13,
    "cheap_hire_max": 21,
    "hire_money_frac": 0.2246,
    "move_factor": 3.3801,
    ...
    "reserve_frac": 0.25,
    "seed_budget_frac": 0.4346,
    "expand_when_empty": 10,
    "cash_comfort": 2500.0,
    "discount_rate": 0.0212,
    "starved_discount": 0.0222,
    "land_last_day": 19,
    "animal_last_day": 20,
    "animal_reserve": 498,
    ...
    # Chaining the jobs on the tile a unit already stands on looks free and is
    # not. Bonusing distance-0 jobs by a multiple of `action_cost` does move the
    # targeted metric -- CARE goes 1.47 tiles a job to 0.73 at a bonus of 30 --
    # but total walk per job barely shifts (1.55 -> 1.48) and the bank falls the
    # whole way, -$5,445 against v23 at a bonus of 2.0. Travel is conserved:
    # chaining only decides which job gets the free slot, and it spends it on
    # the cheap one. Kept at 0; the constant stays so the result is not
    # rediscovered.
    "same_tile_bonus": 0.0,
    ...
    # "ggrr": rank by net value while the farm is one or two quadrants, then by
    # net value *per turn committed* once it spans three or four. Held out over
    # 128 games a side against both benchmarks: 0.824 and +$6,031, against 0.703
    # and +$4,996 for the nearest-based plan and roughly 0.55/+$1,081 for greedy
    # throughout. Chosen on win rate rather than margin because that is what the
    # ladder's rating is computed from; `grrr` banks more (+$7,608) at 0.785.
    #
    # `rate` used from the first quadrant is catastrophic (-$40,732): with the
    # whole farm inside a few tiles the commitment denominator is nearly
    # constant, so dividing by it only amplifies noise. It earns its place only
    # once distances actually vary.
    "assignment_plan": "grrn",
    "assignment_fanout": 6,
    ...
    "plant_commitment_cost": 20.9471,
    "town_pull_weight": 0.1,
    ...
    "rival_supply_weight": 0.312,
    # Buying land just because the bank is full measures -$3,486. Off.
    "expand_when_rich": 1e9,
    # Letters from land_is_worth_working: c capacity, s stocked, d demand.
    # "" keeps the cash-only rule the agent has always used.
    "land_policy": "c",
    # Do not buy a quadrant while the farm already owes half again more work
    # than its hands can reach. The threshold is the whole finding: at 1.0 it
    # blocks constantly and costs $9k, at 1.65 it never binds, and on the 1.35
    # to 1.5 plateau it is worth +$5,612 and +0.08 win rate over buying on cash
    # alone. Affording a quadrant and being able to work one are separate
    # questions and only the first was ever asked.
    "land_capacity_slack": 1.35,
    "land_free_pens": 2,
    "land_demand_units": 120.0,
...
ASSIGNMENT_CODES = {"g": "greedy", "o": "optimal", "n": "nearest",
                    "j": "jobfirst", "r": "rate"}
...
def _order_pairs(mode, pairs, jobs, n_units, params, seeds, stock, carrying):
    ...
    * ``optimal``  -- maximise the turn's total assigned value.
    ...
    if mode == "rate":
        # Net value per turn the assignment actually occupies. `greedy` charges
        # a toll for walking and then compares totals, which prices one turn of
        # a decision that commits a unit for `dist + 1` of them: a $400 job four
        # tiles away and a $340 job next door score the same, though the second
        # frees the unit four turns sooner to earn again. Dividing by the
        # commitment is the return-rate form of the same trade.
        return sorted(pairs, key=lambda p: (-(p[0] / (1.0 - p[1])), p[1], p[2]))
    return pairs
...
def land_is_worth_working(farm, info, shed, params, market_inventory, day, pull):
    """Whether a new quadrant is land we can actually put to work.

    Affording a quadrant and being able to use one are different questions, and
    only the first was ever asked. A farm already behind on watering, holding
    animals it has not placed, or growing into demand that is already covered
    does not need more ground -- it needs to finish what it started. Buying
    anyway spreads the same hands thinner over more tiles, and every one of
    those tiles then wants watering for the rest of the season.

    `land_policy` is a set of letters so the tests compose:
      c -- capacity: we are keeping up with the work we already own
      s -- stocked:  no animal waiting in the shed, few pens standing empty
      d -- demand:   the town still wants more than our fields will deliver
    """
    policy = params["land_policy"]
    if not policy:
        return True

    if "c" in policy:
        if labour_saturation(farm, info, params) > params["land_capacity_slack"]:
            return False

    if "s" in policy:
        waiting = sum(shed.get(name, 0) for name in ANIMALS)
        if waiting > 0:
            return False
        if len(info["empty_structures"]) > params["land_free_pens"]:
            return False

    if "d" in policy:
        # Demand still unclaimed by what is already in the ground.
        spare = 0.0
        for crop in CROPS:
            units, _occupancy = realizable(crop, day)
            if units <= 0:
                continue
            committed = info["planted"].get(crop, 0) * units
            spare += max(0.0, (pull or {}).get(crop, 0.0) - committed)
        if spare < params["land_demand_units"]:
            return False
...
def plan_market(obs, farm, private, params, shed, day, hour, step, info,
    ...
    def buy_land(budget, slots):
        ...
        if not land_is_worth_working(farm, info, shed, params, market_inventory,
                                     day, pull):
            return budget, slots, False
